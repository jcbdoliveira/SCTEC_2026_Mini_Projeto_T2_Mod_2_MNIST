...
def visualizar_balanceamento_classes(X, y, modelo):
    ...
    plt.close()
    print(f"Gráfico salvo em : 'images/{modelo}/balanceamento_classes.png'")
...
def processamento_e_separacao(X, y, modelo='V1'):
    ...
    if modelo == 'V2':
        # ---- Geração dos Traços Finos (Erosão) ----
        print("-> Gerando imagens com traços finos (Erosão)...")
        x_treino_extra = []
        kernel = np.ones((2, 2), np.uint8) # Define a intensidade do afinamento
        
        for img in x_treino_cnn:
            # O OpenCV precisa da imagem sem a dimensão do canal para a erosão
        ...
        # ---- NOVO: Geração dos Traços Encorpados e Suavizados (Dilação + Blur) ----
        print("-> [V3] Gerando imagens com traços robustos (Dilação) e suavizados (Blur)...")
        x_treino_extra = []
        kernel = np.ones((2, 2), np.uint8) # Define a intensidade do engrossamento
        
        for img in x_treino_cnn:
            # Como a imagem original já está em escala 0.0-1.0, convertemos temporariamente para 0-255 para o OpenCV
            img_255 = (img[:, :, 0] * 255).astype(np.uint8)
            
            # 1. Aplica dilação para engrossar a linha da escrita manual
            img_dilated = cv2.dilate(img_255, kernel, iterations=1)
            
            # 2. Aplica suavização gaussiana para recriar as bordas esfumaçadas do MNIST
            img_blurred = cv2.GaussianBlur(img_dilated, (3, 3), 0)
            
            # 3. Normaliza de volta para float32 entre 0.0 e 1.0 e reinsere o canal de cor
    ...
    # Retornamos os dados de treino já expandidos e combinados
    if modelo in ['V2', 'V3']:
        print(f"-> Treino Expandido {modelo} (Original + Modificado): {x_treino_expandido.shape[0]} amostras | Formato CNN: {x_treino_expandido.shape}")            
        print(f"-> Teste (Mantido original):  {x_teste_cnn.shape[0]} amostras | Formato CNN: {x_teste_cnn.shape}")
        
        # CORREÇÃO PARA RF/SVM: Sincroniza o x_treino_norm com as imagens duplicadas achatando a matriz expandida
...
def treinar_modelo_CNN(x_treino_cnn, y_treino, modelo):
    ...
    checkpoint_cb = callbacks.ModelCheckpoint(
        filepath=os.path.join("models", modelo, "cnn_mnist.keras"),
        save_best_only=True,
        monitor='val_loss',
        mode='max'
    )
